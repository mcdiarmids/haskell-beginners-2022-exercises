...
lecture2Normal = describe "Normal" $ do
  ...
  describe "dragons" $ do
    it "Knight Red Dragon Victory" $
      dragonFight knight redDragon
        `shouldBe` Victory (Reward {rewardExperience = Experience 100, rewardTreasure = TreasureChest rubyChest})
    it "Knight Black Dragon Victory" $
      dragonFight knight blackDragon {dragonHealth = Health 200}
        `shouldBe` Victory (Reward {rewardExperience = Experience 150, rewardTreasure = TreasureChest sapphireChest})
    it "Knight Green Dragon Victory" $
      dragonFight knight greenDragon {dragonHealth = Health 10}
        `shouldBe` Victory (Reward {rewardExperience = Experience 250, rewardTreasure = TreasureGold (Gold 1000)})

    it "Knight Escape" $ dragonFight knight blackDragon `shouldBe` Escape
    it "Knight Defeat" $ dragonFight knight greenDragon `shouldBe` Defeat
...
lecture2Hard = describe "Hard" $ do
  ...
  describe "merge" $ do
    ...
    it "merge (sort xs) (sort ys) ≡ sort (xs ++ ys)" $
      hedgehog $ do
        xs <- forAll $ Gen.list (Range.linear 0 10) (Gen.int Range.linearBounded)
  ...
  describe "constantFolding" $ do
    ...
    it "Var and Lit 5" $
      constantFolding (Add (Var "x") (Lit 5))
        `isOneOf` [ Add (Var "x") (Lit 5),
                    Add (Lit 5) (Var "x")
                  ]
...
knight =
  Knight
    { knightHealth = Health 700,
      knightAttack = Attack 30,
      knightEndurance = Endurance 15
    }

redDragon :: Dragon Gemstone
redDragon =
  Dragon
    { dragonHealth = Health 250,
      dragonAttack = Attack 20,
      dragonFirePower = Attack 50,
      dragonType = RedDragon rubyChest
    }

blackDragon :: Dragon Gemstone
blackDragon =
  Dragon
    { dragonHealth = Health 650,
      dragonAttack = Attack 35,
      dragonFirePower = Attack 60,
      dragonType = BlackDragon sapphireChest
    }

greenDragon :: Dragon Gold
greenDragon =
  Dragon
    { dragonHealth = Health 900,
      dragonAttack = Attack 80,
      dragonFirePower = Attack 150,
      dragonType = GreenDragon (Gold 1000)
    }
...
sapphireChest :: Chest Gemstone
sapphireChest =
  Chest
    { chestTreasure = Sapphire,
      chestGold = Gold 500
    }
