...
-- does not terminate on infinite leading space
...
dragonFight = go 1
  where
    ...
    go turn k d
      | unHealth (knightHealth k) <= 0 = Defeat
      | unEndurance (knightEndurance k) == 0 = Escape
      | unHealth (dragonHealth d) <= 0 = Victory (calculateReward d)
      | otherwise =
        go
          (turn + 1)
          k {knightHealth = damage dAtk (knightHealth k), knightEndurance = decreaseEndurance (knightEndurance k)}
          d {dragonHealth = damage (knightAttack k) (dragonHealth d)}
      where
        dAtk = if turn `mod` 10 == 0 then dragonFirePower d else dragonAttack d

    damage :: Attack -> Health -> Health
    damage (Attack atk) (Health hp) = Health (hp - atk)

    decreaseEndurance :: Endurance -> Endurance
    decreaseEndurance endurance = Endurance (unEndurance endurance - 1)

calculateReward :: Dragon a -> Reward a
calculateReward dragon = case dragonType dragon of
  RedDragon chest -> Reward {rewardExperience = Experience 100, rewardTreasure = TreasureChest chest}
  BlackDragon chest -> Reward {rewardExperience = Experience 150, rewardTreasure = TreasureChest chest}
  GreenDragon gold -> Reward {rewardExperience = Experience 250, rewardTreasure = TreasureGold gold}
...
eval vars (Var v) = case lookup v vars of
  ...
  Just x -> Right x
eval vars (Add ea eb) = addExpressions (eval vars ea) (eval vars eb)
  where
    addExpressions :: Either EvalError Int -> Either EvalError Int -> Either EvalError Int
    addExpressions ea' eb' = case ea' of
      Left err -> Left err
      Right a -> case eb' of
        Left err -> Left err
        Right b -> Right (a + b)
...
constantFolding expr = case collect expr (0, []) of
  (c, []) -> Lit c
  (0, vs) -> foldr1 Add vs
  (c, vs) -> foldr1 Add (Lit c : vs)
  where
    collect :: Expr -> (Int, [Expr]) -> (Int, [Expr])
    collect (Lit n) (s, vs) = (s + n, vs)
    collect (Var v) (s, vs) = (s, Var v : vs)
    collect (Add ea eb) (s, vs) = add (s, vs) $ add (collect ea (0, [])) (collect eb (0, []))

    add :: (Int, [Expr]) -> (Int, [Expr]) -> (Int, [Expr])
    add (t1, vs1) (t2, vs2) = (t1 + t2, vs1 ++ vs2)
